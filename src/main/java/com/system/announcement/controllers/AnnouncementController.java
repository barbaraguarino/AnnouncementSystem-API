...
public class AnnouncementController {
    ...
    public ResponseEntity<Object> createAnnouncement(@RequestBody @Valid SaveAnnouncementDTO saveAnnouncementDTO) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(announcementService.save(saveAnnouncementDTO));
    }

    @PostMapping("/edit/{id}")
    public ResponseEntity<Object> editAnnouncement(@RequestBody @Valid SaveAnnouncementDTO saveAnnouncementDTO,
                                                   @PathVariable @NotNull UUID id) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(announcementService.editById(saveAnnouncementDTO, id));
    }

    @PostMapping("/filter")
    public ResponseEntity<Object> filterAnnouncements(@RequestBody @Valid FilterAnnouncementDTO filterDTO,
                                                      Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(announcementService.findAllWithFilter(filterDTO, pageable));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Object> getAnnouncementById(@Valid @PathVariable UUID id) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(announcementService.findById(id));
    }

    @GetMapping("/closed")
    public ResponseEntity<Object> getClosedAnnouncements(Pageable pageable){
        return ResponseEntity.status(HttpStatus.OK)
                .body(announcementService.findAllClosed(pageable));
    }

    @GetMapping("/suspended")
    public ResponseEntity<Object> getSuspendedAnnouncements(Pageable pageable){
        return ResponseEntity.status(HttpStatus.OK)
                .body(announcementService.findAllSuspended(pageable));
    }

    @GetMapping("/open")
    public ResponseEntity<Object> getOpenAnnouncements(Pageable pageable){
        return ResponseEntity.status(HttpStatus.OK)
                .body(announcementService.findAllOpen(pageable));
    }
